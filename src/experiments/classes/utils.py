...
def index_to_class_dict():
	# Match the class numbers to their corresponding RVL-CDIP class name
	return {
		0: "letter",
		1: "form",
		2: "email",
...
def class_to_index_dict():
	# Match the classes names to their corresponding RVL-CDIP class number
	return {
		"letter": 0,
		"form": 1,
		"email": 2,
...
def classes_list():
	# Juste the RVL-CDIP classes list with positions matching classes indexes
...
class CustomResizeTransform(object):
	# This class can be used as a custom PyTorch transform for CNN models
